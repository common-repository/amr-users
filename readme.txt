...
== Changelog ==
= 4.59.4 (2022 03 31) =
*  Added more missing esc_attr to protect from XSS in admin (missing/glitch in previous update)

= 4.59.3 (2022 03 27) =
*  Added missing esc_attr to protect from XSS in admin

= 4.59.2 (2022 03 26) =
*  Tightened up admin settings input sanitisation 

= 4.59.1 =
*  Added a new filter 'amr_users_before_format_value' that is applied to the field value before any formatting.  This allows field values to be translated before being formatted.  AS requested in order to use wpml to do string translation of user fields like bio/description.  No need to update if not using wpml.
*  Also added a fix to the license check for EDD plugin license lifetime expiry bug

= 4.59 =
*  improved search of users. Previously two terms would search for either.  Now one can search for a phrase by putting double quotes around it.   Double quoting a single word will cause plugin to search where that is the full value of the field (not part of it).   Using -term will exclude users who have that term anywhere in the list record.

= 4.58 =
*  added options to handle custom post types and taxonomies linked to users, not just as single ids but also as arrays of those.  This was in response to buddypress facilitating these links in their data tables.  Functionality should work for such post and taxonomy ids in the wp user meta. In the latest amr-users-plus formatting functions are . See https://bp.wpusersplugin.com/user-cpt-and-taxo/

= 4.57 =
*  tweak some error messages for list import tools
*  testing on wp 5.8-RC4

= 4.56 =
*  tweak for lists that start empty.   If empty, column headings will not show.  If records are returned as result of search, or filter then headings will show.
*  a css class of empty is added for the table and the totals div.
*  some minor code tweaks for translations and if lists are deleted but shortcode still in use.
*  testing on php 8
...
After activation default lists will be available. 
  
1.  Create sample data for your users for any new fields.  
2.  Go to Settings, see 'find fields'.  Click 'Find Fields' to find any extra user fields you may have now that you have some sample data.  If your fields are not found, you are probably using a third party plugin that does not store it's data in the wordpress user meta.  There are add-ons for these.  See at http://wpusersplugin.com/
3.  Configure the default lists or add more listings.   Your newly found fields will be available when you configure the list.  Blank out the 'display order' etc to remove fields you do not want. Scroll down until you see the fields that you do want.  Type a number into the display order.  Blank or add '1' to the 'sort order' for the fields.   Click Update.   Click 'Rebuild cache now'.  
4.  To display lists on the front end, create a page or post, enter the shortcode in text [userlist list=n].  
